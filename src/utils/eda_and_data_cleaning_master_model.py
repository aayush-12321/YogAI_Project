...
METADATA_COLUMNS = ['source_id', 'label', 'original_label', 'frame_number']
...
def _extract_biomechanical_features(df, metadata_cols=METADATA_COLUMNS):
    ...
    cols = [col for col in df.columns if col not in metadata_cols]
    # Select only numeric types (float, int) to avoid errors/slowdowns with string/categorical columns
...
def plot_feature_variance_by_class(df, pose_name="Pose", showfliers=False):
    """
    ...
    By default, outliers (fliers) are hidden to prevent significant Matplotlib rendering slowdowns.
    """
    biomechanical_features = _extract_biomechanical_features(df)
    ...
    for idx, (title, features) in enumerate(plots_to_make):
        ...
        sns.boxplot(data=melted_df, x='Value', y='Feature', hue='label', palette='Set2', ax=ax, showfliers=showfliers)
...
def plot_signal_stability_over_time(df, target_source_id=None, pose_name="Pose"):
    ...
    Safeguards against very long sequences (e.g., unified pose labels) by truncating the plot.
    """
    biomechanical_features = _extract_biomechanical_features(df)
    ...
    if len(sequence_df) > 1000:
        print(f"[ INFO] Sequence is too long ({len(sequence_df)} frames). Truncating plot to first 1000 frames for performance.")
        sequence_df = sequence_df.head(1000)
